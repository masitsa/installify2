...
class Blog extends admin {
	...
	*/
	public function comments($post_id = NULL) 
	{
		...
		//pagination
		$this->load->library('pagination');
		$config['base_url'] = base_url().'blog/comments/'.$post_id;
		$config['total_rows'] = $this->users_model->count_items($table, $where);
		...
		if ($query->num_rows() > 0)
		{
			...
			$data['title'] = $v_data['title'] = $this->blog_model->get_comment_title($post_id);
	...
	public function add_comment($post_id) 
	{
		...
		{	
			if($this->blog_model->add_comment_admin($post_id))
			{
				...
				redirect('blog/comments/'.$post_id);
			}
		...
		$v_data['post_id'] = $post_id;
		$v_data['title'] = $data['title'] = $this->blog_model->get_comment_title($post_id);
